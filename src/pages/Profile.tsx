...
const Profile: React.FC = () => {
  ...
  // Fetch user profile
  const { data: profile, isLoading, error, refetch } = useGetMyProfileQuery(undefined, {
    skip: !token,
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
              ...
              <div className="flex-1 pt-4">
                <div className="flex items-center gap-3 mb-2">
                  <h1 className="text-2xl font-bold text-gray-900">
                    {profile?.companyName || profile?.fullName || 'Пользователь'}
                  </h1>
                  <div className="flex gap-2">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${getBadgeColor(profile?.businessType || '')}`}>
                      {profile?.businessType === 'carrier' ? 'Перевозчик' : 'Грузовладелец'}
                    </span>
                    {profile?.isVerified && (
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${getBadgeColor('verified')}`}>
                        ✓ Верифицирован
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  <span>🏢 {profile?.type === 'legal_entity' ? 'Юридическое лицо' : 'Физическое лицо'}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 text-center">
                  <div className="text-2xl font-bold text-purple-600">{profile?.rating?.toFixed(1) || '0.0'}</div>
            ...
            <div className="space-y-6">
              ...
              {/* Location */}
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">📍 Местоположение</h3>
                <div className="space-y-2 text-sm">
                  <div><strong>Страна:</strong> {profile?.country}</div>
                  <div><strong>Почтовый код:</strong> {profile?.postalCode}</div>
                  <div><strong>Адрес:</strong> {profile?.address}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'settings' && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">⚙️ Настройки профиля</h2>
            
            <div className="text-center py-12 text-gray-500">
              <div className="text-4xl mb-4">⚙️</div>
              <p className="mb-4">Для изменения настроек профиля перейдите на страницу редактирования</p>
              <button
                onClick={() => navigate('/profile/edit')}
                className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"
              >
            ...
            <div className="space-y-6">
              ...
              <div className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-medium text-gray-900">Статус верификации</h3>
                    <p className="text-sm text-gray-600">Ваш аккаунт {profile?.isVerified ? 'верифицирован' : 'не верифицирован'}</p>
                  </div>
              ...
              {/* Change Password */}
              <div className="border border-gray-200 rounded-lg p-4">
                <h3 className="font-medium text-gray-900 mb-2">Изменить пароль</h3>
                <p className="text-sm text-gray-600 mb-4">Регулярно обновляйте пароль для безопасности</p>
              ...
              <div className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Дополнительная защита для вашего аккаунта</p>
                  </div>
              ...
              {/* Session Management */}
              <div className="border border-gray-200 rounded-lg p-4">
                <h3 className="font-medium text-gray-900 mb-2">Управление сессиями</h3>
                <p className="text-sm text-gray-600 mb-4">Просмотр и управление активными сессиями</p>
