...
const DetailedTruck: React.FC = () => {
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              {t.detailedTruck?.notFoundDesc || "Грузовик с указанным ID не существует или был удален"}
            </p>
            <Button
  ...
  const getTruckOptionName = () => {
    ...
    return typeof truck.truckOption === "string" ? truck.truckOption : (t.common?.notSpecified || "Не указан");
  };
  ...
  const formatDimensions = (dimensions?: { length?: number; width?: number; height?: number }) => {
    if (!dimensions || typeof dimensions !== "object") return t.common?.notSpecified || "Не указаны";
  ...
  const getReadyTypeInfo = () => {
    if (!truck.readyType?.type) return { text: t.common?.notSpecified || "Не указан", color: "gray" };
  ...
  const getPriceInfo = () => {
    if (!truck.pricing) return { amount: t.common?.negotiable || "Договорная", type: "", vat: "" };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="p-4 bg-blue-50 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-blue-900">{truck.loadCapacity} т</p>
                  </div>
                  ...
                  <div className="p-4 bg-indigo-50 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-indigo-900">{truck.volume} м³</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {(truck.trailerType === "trailer" || truck.trailerType === "semi_trailer") && truck.trailer_dimensions && (
                    <div className="p-4 bg-purple-50 rounded-lg">
                      <div className="flex items-center gap-2 text-purple-700 font-medium mb-2">
                        <Ruler size={16} />
                        {t.postTruck?.specifications?.trailerDimensions || "Размеры прицепа"}
                      </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-3">
                    {t.postTruck?.truckDetails?.supportedLoadingMethods || "Поддерживаемые типы загрузки"}
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {truck.loadTypes && truck.loadTypes.length > 0 ? (
                      ...
                      <span className="text-gray-500">{t.common?.notSpecified || "Не указаны"}</span>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-3">
                    {t.postTruck?.specifications?.specialFeatures || "Дополнительные возможности"}
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className={`p-4 rounded-lg border-2 ${
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-medium ${
                          truck.features?.hydraulicLift ? "text-green-800" : "text-gray-600"
                        }`}>
                    ...
                    {truck.gpsMonitoring?.enabled ? (
                      ...
                      <>
                        ...
                        <span className="text-gray-600">
                          {t.detailedTruck?.gpsNotAvailable || "GPS мониторинг не доступен"}
                        </span>
                      </>
                    )}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-green-800 font-medium">
                      📍 {truck.fromAddress?.display_place || t.common?.notSpecified || "Не указано"}
                    </p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                    <p className="text-red-800 font-medium">
                      📍 {truck.toAddress?.display_place || t.common?.notSpecified || "Не указано"}
                    </p>
              ...
              <div className="mt-6 p-6 bg-blue-50 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  {readyTypeInfo.icon && (
                    <readyTypeInfo.icon 
                      className={`${
                        readyTypeInfo.color === "green" ? "text-green-600" : 
                        readyTypeInfo.color === "yellow" ? "text-yellow-600" : 
                        "text-gray-600"
                      }`} 
                      size={20} 
                    />
                  ...
                  <span className={`font-medium ${
                    readyTypeInfo.color === "green" ? "text-green-800" : 
                    readyTypeInfo.color === "yellow" ? "text-yellow-800" : 
                    "text-gray-800"
                  }`}>
                    {readyTypeInfo.text}
                  </span>
                ...
                {truck.readyType?.interval && truck.readyType.type === "ready_from" && (
                  <p className="text-blue-700 text-sm mt-2">
                    {t.detailedTruck?.frequency || "Частота"}: {
                      t.postTruck?.intervals?.[truck.readyType.interval as keyof typeof t.postTruck.intervals] || 
          ...
          <div className="space-y-8">
                ...
                {truck.pricing?.cash && truck.pricing.cash > 0 && (
                  <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-green-800 font-medium text-sm">
                      💰 {t.detailedTruck?.cashPayment || "Наличные"}: {truck.pricing.cash.toLocaleString()}
                    </p>
                  </div>
                )}
                  ...
                  <>
                    <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">
                          {profile.companyName || 
                           (typeof truck.owner === "object" ? truck.owner.fullName : null) ||
                    ...
                    <div className="pt-4 border-t border-gray-200">
                      <p className="text-gray-500 text-sm">
                        {t.detailedTruck?.memberSince || "Участник с"}: {profile.createdAt ? new Date(profile.createdAt).toLocaleDateString() : t.common?.notSpecified || "Не указано"}
                      </p>
                    </div>
                  </>
                )}
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-xl font-semibold mb-4">
                {t.detailedTruck?.contact || "Связаться"}
              </h2>
              <div className="space-y-3">
                <Button className="w-full bg-blue-600 hover:bg-blue-700">
                  <Phone size={16} className="mr-2" />
                  {t.detailedTruck?.callOwner || "Позвонить владельцу"}
                </Button>
                <Button variant="outline" className="w-full">
                  <Mail size={16} className="mr-2" />
                  {t.detailedTruck?.sendMessage || "Отправить сообщение"}
                </Button>
              </div>
            </div>
          </div>
